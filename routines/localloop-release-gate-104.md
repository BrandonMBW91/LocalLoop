...
- If you see "✔ Flipped" then 1.0.4 IS LIVE — the in-app prompt is armed and the push was sent. Say so clearly (include how many devices got the push), and note that this scheduled task has done its job and can be turned off (list_scheduled_tasks then delete_scheduled_task for taskId "localloop-release-gate-104").
