...
export default function DealCard({ deal }) {
  const { backendEnabled, scale, noTrack } = useApp();
  ...
  const open = () => {
    // !noTrack, same as AdBanner: taps is the ONE number a $9/mo deal advertiser is
    // sold on, so it must exclude admin/dev/opted-out. Without this, checking a
    // customer's own link counted as a tap and inflated what we report back to them.
    if (backendEnabled && !noTrack && deal.id) recordDealView(deal.id);
